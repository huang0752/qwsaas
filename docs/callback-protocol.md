...
Outbound response correlation is strong only when `parse_sent_message_ref()` finds a valid `data.msg_data.appinfo` equal to the callback's `appinfo`. `send_flag`, timestamps, text, `id`, or `seq` alone do not prove an API echo.
